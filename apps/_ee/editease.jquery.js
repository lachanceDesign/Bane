...
function doLogin(login) {
 jQuery("#eesbt").attr({ disabled: "disabled" });
...
function eeLive(){
 jQuery.post(""+eePath+"/eepanel.php?val="+Math.random(),function(data){jQuery("body").prepend(data);eeActive();});
...
function editNow(val){
		...
		jQuery(vID).before('<div id="ma-'+val+'" class="emask" style="height:'+vh+'px;width:'+vw+'px;"></div>');
				...
				var jQuerycontents = jQuery.post(""+eePath+"/getfile.php",{file:val,height:vh,width:vw,eType:veditor,eData:"edit"}, function(data){
...
function eeSave(tVal){
		...
		jQuery(vID).before('<div id="ma-'+fval[1]+'" class="emask" style="height:'+vh+'px;width:'+vw+'px;"></div>');
				var jQuerycontents = jQuery.post(""+eePath+"/getfile.php",{file:fval[1],eData:"save",nData:nData}, function(data){
...
function eeCancel(tVal){
    ...
    $.prettyPhoto.close();
		...
		var vw = jQuery(vID).outerWidth();
		jQuery(vID).before('<div id="ma-'+fval[1]+'" class="emask" style="height:'+vh+'px;width:'+vw+'px;"></div>');
				var jQuerycontents = jQuery.post(""+eePath+"/getfile.php",{file:fval[1],eData:"cancel"}, function(data){
						jQuery('#'+fval[1]).html(data);
						jQuery('#'+fval[1]).removeClass("edithoverActive");
						jQuery('#ed-'+fval[1]).fadeOut("slow");
						jQuery('#ee-'+fval[1]).fadeIn("medium");
...
function closeEditing(tVal,nData){
	...
	$.prettyPhoto.close();
		if(nData!=='xxlee'){
				var jQuerycontents = jQuery.post(""+eePath+"/getfile.php",{file:tVal,eData:"save",nData:nData}, function(data){
